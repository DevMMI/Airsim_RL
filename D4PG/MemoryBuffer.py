...
class MemoryBuffer:
    ...
    def __init__(self, sess, coord):
        ...
        self.buffer_s = np.empty([Settings.BUFFER_SIZE, *Settings.STATE_SIZE], dtype=np.float32) #state
        self.buffer_a = np.empty([Settings.BUFFER_SIZE, Settings.ACTION_SIZE], dtype=np.float32) #action
        self.buffer_r = np.empty([Settings.BUFFER_SIZE], dtype=np.float32) #reward
        self.buffer_s_ = np.empty([Settings.BUFFER_SIZE, *Settings.STATE_SIZE], dtype=np.float32) #next state
        self.buffer_d = np.empty([Settings.BUFFER_SIZE], dtype=np.float32) #priority?
